    ...
    def build(self, slots: list[tuple[Engine, tuple]], vliw: bool = False):
        ...
        for i in range(n):
            for d in deps[i]:
                succs[d].add(i)
                indeg[i] += 1
        # Criticality: longest path to end (heuristic for scheduling).
        order = sorted(range(n), key=lambda i: len(succs[i]))
        crit = [1] * n
        for i in sorted(order, reverse=True):
            if succs[i]:
                crit[i] = 1 + max(crit[s] for s in succs[i])
        ...
        while remaining:
            ...
            # If the next ready instr is debug, emit it alone.
            ...
            while progressed:
                progressed = False
                for i in sorted(ready, key=lambda k: (-crit[k], k)):
                    engine, slot = slots[i]
                    reads = reads_list[i]
            ...
            # Fallback: schedule the smallest ready instruction to make progress.
            i = min(ready)
            engine, slot = slots[i]
            instrs.append({engine: [slot]})
            ready.remove(i)
