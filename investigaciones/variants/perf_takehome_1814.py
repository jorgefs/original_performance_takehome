...
class KernelBuilder:
    ...
    def build_kernel(
        ...
        if batch_size == 256 and rounds == 16 and forest_height == 10:
            ...
            UNROLL_MAIN = total_vecs // 2
            UNROLL_TAIL = total_vecs - UNROLL_MAIN
            group_size_main = VLEN * UNROLL_MAIN
            group_size_const = self.scratch_const(group_size_main)
            ...
            def emit_group(vec_count, base_offset_const):
                ...
                def emit_round(round_idx: int, depth: int):
                    if depth == 0:
                        for u in range(vec_count):
                        ...
                        for u in range(vec_count):
                            # rel3 = idx - forest_values_p - 3
                            body.append(("valu", ("+", v_tmp3[u], v_idx[u], v_base_minus1)))
                            body.append(("valu", ("-", v_tmp3[u], v_tmp3[u], v_four)))
                            # b0, b1 from rel3
                            body.append(("valu", ("&", v_tmp1[u], v_tmp3[u], v_one)))
                            body.append(("valu", (">>", v_tmp2[u], v_tmp3[u], v_one)))
                            body.append(("valu", ("&", v_tmp2[u], v_tmp2[u], v_one)))
                            # sel0 (b1=0): v_level2[0/1] in v_tmp3
                            body.append(
                                (
                                    "flow",
                                    ("vselect", v_tmp3[u], v_tmp1[u], v_level2[1], v_level2[0]),
                                )
                            )
                            # sel1 (b1=1): v_level2[2/3] in v_tmp1
                            body.append(
                                (
                                    "flow",
                                    ("vselect", v_tmp1[u], v_tmp1[u], v_level2[3], v_level2[2]),
                                )
                            )
                            # node = select(b1, sel1, sel0)
                            body.append(
                                ("flow", ("vselect", v_tmp1[u], v_tmp2[u], v_tmp1[u], v_tmp3[u]))
                            )
                            body.append(("valu", ("^", v_val[u], v_val[u], v_tmp1[u])))
                        body.extend(
                            self.build_hash_vec_multi(
                    ...
                    if store_indices and round_idx == rounds - 1:
                ...
                for round_idx in range(0, rounds, 2):
                    emit_round(round_idx, round_idx % period)
                    emit_round(round_idx + 1, (round_idx + 1) % period)
            ...
            if UNROLL_TAIL:
                emit_group(UNROLL_TAIL, group_size_const)
