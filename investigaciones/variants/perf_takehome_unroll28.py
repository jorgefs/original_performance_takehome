"""
Optimization: UNROLL=28 with precomputed partial_idx.

By reducing UNROLL from 32 to 28, we free up enough scratch space
to add v_partial_idx vectors. This allows precomputing:
  partial_idx = idx * 2 + (1 - fp)
BEFORE the hash, so it can run in parallel with hash computation.

After hash, we only need:
  bit = val & 1
  idx = partial_idx + bit

This moves the multiply_add from the post-hash critical path.
"""
...
class KernelBuilder:
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        if batch_size == 256 and rounds == 16 and forest_height == 10:
            ...
            # REDUCED: UNROLL=28 to make room for v_partial_idx
            UNROLL_MAIN = 28
            TAIL_VECS = 4  # 32 - 28 = 4 vectors for tail
            ...
            # NEW: Precomputed partial idx vectors
            v_partial_idx = [self.alloc_scratch(f"v_partial{u}", length=VLEN) for u in range(UNROLL_MAIN)]
            ...
            def emit_finalize_idx_update(start, count):
                """Finalize idx update: bit = val & 1, idx = partial + bit."""
                for u in range(start, start + count):
                    body.append(("valu", ("&", v_tmp1[u], v_val[u], v_one)))
                    body.append(("valu", ("+", v_idx[u], v_partial_idx[u], v_tmp1[u])))

            def emit_full_idx_update(start, count):
                """Full 3-op idx update (when no precomputation available)."""
                for u in range(start, start + count):
                    body.append(("valu", ("&", v_tmp1[u], v_val[u], v_one)))
            ...
            def emit_hash_only_range(round_idx, depth, start, count):
                ...
                if depth == 0:
                    ...
                    # Precompute partial for next round (depth 2)
                    ...
                    for u in range(count):
                        ...
                        body.append(("valu", ("^", v_val_l[u], v_val_l[u], v_tmp1_l[u])))
                    # Precompute partial for next round (depth 3)
                    emit_precompute_partial_idx(start, count)
                    body.extend(self.build_hash_vec_multi(v_val_l, v_tmp1_l, v_tmp2_l,
                                                          round_idx, start * VLEN, emit_debug))
                else:
                    # Depths 3-10: Precompute partial BEFORE loads (if not depth 10)
            ...
            def emit_group(vec_count, base_offset_const, base_is_zero=False):
                ...
                for round_idx in range(rounds):
                    ...
                    chunk = 1
                    if depth > 2:
                        starts = tuple(range(0, vec_count, 1))  # Sequential for simplicity
                    else:
                        starts = tuple(range(0, vec_count, 1))
                    ...
                    for start in starts:
                        ...
                        if depth != 0 and depth != forest_height:
                            # Use precomputed partial if available (from previous round)
                            # Partial is computed during depths 1, 2, 3-9 for the NEXT round
                            # So at depths 2-10 we can use it, except depth 10 doesn't need idx update
                            if prev_depth >= 1 and prev_depth != forest_height:
                                # Precomputed partial is available
                                emit_finalize_idx_update(start, count)
                            else:
                                # First round after wrap (depth 1) or no precompute available
                                emit_full_idx_update(start, count)
            ...
            # Process main group (28 vectors = 224 elements)
            emit_group(UNROLL_MAIN, zero_const, base_is_zero=True)

            # Process tail (4 vectors = 32 elements) - reuse v_idx[0:4], v_val[0:4], etc.
            tail_offset_const = self.scratch_const(UNROLL_MAIN * VLEN)

            # Setup tail addresses
            body.append(("alu", ("+", tmp_val_addr_u[0], self.scratch["inp_values_p"], tail_offset_const)))
            if store_indices:
                body.append(("alu", ("+", tmp_idx_addr_u[0], self.scratch["inp_indices_p"], tail_offset_const)))
            ...
            for round_idx in range(rounds):
                ...
                # Simplified emit for tail (no precomputation to keep it simple)
                if depth == 0:
                    for u in range(TAIL_VECS):
                        body.append(("valu", ("^", v_val[u], v_val[u], v_root_val)))
                    body.extend(self.build_hash_vec_multi(v_val[:TAIL_VECS], v_tmp1[:TAIL_VECS],
                                                          v_tmp2[:TAIL_VECS], round_idx,
                                                          UNROLL_MAIN * VLEN, emit_debug))
                elif depth == 1:
                    for u in range(TAIL_VECS):
                        body.append(("valu", ("&", v_tmp1[u], v_val[u], v_one)))
                        body.append(("valu", ("+", v_idx[u], v_base_plus1, v_tmp1[u])))
                        body.append(("flow", ("vselect", v_tmp1[u], v_tmp1[u],
                                              v_level1_right, v_level1_left)))
                    ...
                    for u in range(TAIL_VECS):
                        ...
                        body.append(("flow", ("vselect", v_tmp3_shared, v_tmp1[u],
                                              v_level2[3], v_level2[2])))
                        body.append(("flow", ("vselect", v_tmp1[u], v_tmp1[u],
                                              v_level2[1], v_level2[0])))
                        body.append(("flow", ("vselect", v_tmp1[u], v_tmp2[u],
                                              v_tmp1[u], v_tmp3_shared)))
                        body.append(("valu", ("^", v_val[u], v_val[u], v_tmp1[u])))
                    body.extend(self.build_hash_vec_multi(v_val[:TAIL_VECS], v_tmp1[:TAIL_VECS],
                                                          v_tmp2[:TAIL_VECS], round_idx,
                                                          UNROLL_MAIN * VLEN, emit_debug))
                else:
                    body.extend(self.build_hash_pipeline_addr(v_idx[:TAIL_VECS], v_val[:TAIL_VECS],
                                                               v_tmp1[:TAIL_VECS], v_tmp2[:TAIL_VECS],
                                                               round_idx, UNROLL_MAIN * VLEN, emit_debug,
                                                               TAIL_VECS, hash_group=3))
        ...
        raise NotImplementedError("Only standard benchmark parameters supported")
