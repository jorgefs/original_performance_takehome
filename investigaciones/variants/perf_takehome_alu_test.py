    ...
    def build_kernel(
        ...
        if batch_size == 256 and rounds == 16 and forest_height == 10:
            ...
            def emit_group(vec_count, base_offset_const, base_is_zero=False):
                # Pre-calculate offset constants for independent address calculation
                offset_consts = []
                for u in range(vec_count):
                    offset_val = u * VLEN
                    if offset_val not in self.const_map:
                        addr = self.alloc_scratch(f'offset_{offset_val}')
                        body.append(('load', ('const', addr, offset_val)))
                        self.const_map[offset_val] = addr
                    offset_consts.append(self.const_map[offset_val])
                ...
                # Calculate addresses independently (no chain dependency)
                for u in range(1, vec_count):
                    body.append(
                        ("alu", ("+", tmp_val_addr_u[u], base_val_addr, offset_consts[u]))
                    )
                    if store_indices:
                        body.append(
                            ("alu", ("+", tmp_idx_addr_u[u], base_idx_addr, offset_consts[u]))
                        )
