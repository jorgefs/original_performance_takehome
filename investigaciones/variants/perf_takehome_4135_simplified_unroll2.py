...
class KernelBuilder:
    ...
    def build_kernel(
        ...
        if batch_size == 256 and rounds == 16 and forest_height == 10:
            ...
            # root value cache removed; use pipeline loads at depth0.
            ...
            self.add("valu", ("-", v_neg_forest, v_base_minus1, v_one))
            ...
            def emit_hash_only_range(round_idx: int, depth: int, start: int, count: int):
                ...
                if depth == 0:
                    for u in range(count):
                        body.append(("valu", ("+", v_idx_l[u], v_forest_values_p, v_zero)))
                    body.extend(
            ...
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 2))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 3))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 4))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 5))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 6))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 7))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 8))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 9))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 10))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 11))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 12))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 13))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 14))
            emit_group(UNROLL_MAIN, self.scratch_const(VLEN * UNROLL_MAIN * 15))
