...
class KernelBuilder:
    ...
    def build_kernel(
        ...
        if batch_size == 256 and rounds == 16 and forest_height == 10:
            ...
            v_cache_key = [
                self.alloc_scratch(f"v_cache_key_{u}", length=VLEN)
                for u in range(UNROLL_MAIN)
            ]
            v_cache_val = [
                self.alloc_scratch(f"v_cache_val_{u}", length=VLEN)
                for u in range(UNROLL_MAIN)
            ]
            tmp_val_addr_u = [
            ...
            def build_hash_pipeline_addr_cached(
                v_idx,
                v_val,
                v_tmp1,
                v_tmp2,
                ...
                for g in range(num_groups):
                    ...
                    if emit_debug:
                        for u in range(group_start, group_start + group_vecs):
                            base = i_base + u * VLEN
                            keys = [
                                (round_idx, base + lane, "node_val")
                                for lane in range(VLEN)
                            ]
                            slots.append(("debug", ("vcompare", v_tmp1[u], keys)))

                    for u in range(group_start, group_start + group_vecs):
                        slots.append(("valu", ("^", v_tmp3[u], v_idx[u], v_cache_key[u])))
                        slots.append(
                            (
                                "flow",
                                (
            ...
            def emit_hash_only(round_idx: int, depth: int, vec_count: int):
                if depth == 0:
                    ...
                    body.extend(
                        build_hash_pipeline_addr_cached(
                            v_idx,
                            v_val,
                            v_tmp1,
                            v_tmp2,
            ...
            def emit_group(vec_count, base_offset_const):
                ...
                for u in range(vec_count):
                    body.append(("valu", ("+", v_cache_key[u], v_zero, v_zero)))
                    body.append(("valu", ("+", v_cache_val[u], v_zero, v_zero)))
