...
class KernelBuilder:
    ...
    def build_kernel(
        ...
        if batch_size == 256 and rounds == 16 and forest_height == 10:
            ...
            v_level3_hot = [
                self.alloc_scratch(f"v_l3h{i}", length=VLEN) for i in range(4)
            ]
            for i, addr in enumerate(v_level3_hot):
                self.add(
                    "alu",
                    (
                        "+",
            ...
            def emit_hash_only_range(round_idx: int, depth: int, start: int, count: int):
                ...
                v_tmp3_l = v_tmp3[start : start + count]
                v_tmp4_l = v_tmp4[start : start + count]
                v_tmp5_l = v_tmp5[start : start + count]
                if depth == 0:
                    ...
                    for u in range(count):
                        body.append(("valu", ("+", v_tmp3_l[u], v_idx_l[u], v_base_minus1)))
                        body.append(("valu", ("-", v_tmp3_l[u], v_tmp3_l[u], v_one)))
                        body.append(("valu", ("-", v_tmp3_l[u], v_tmp3_l[u], v_seven)))
                        body.append(("valu", ("&", v_tmp1_l[u], v_tmp3_l[u], v_one)))  # b0
                        body.append(("valu", ("&", v_tmp2_l[u], v_tmp3_l[u], v_two)))  # b1
                        body.append(("valu", ("&", v_tmp3_l[u], v_tmp3_l[u], v_four)))  # b2
                        body.append(
                            (
                                "flow",
                                (
                        ...
                        for lane in range(VLEN):
                            body.append(
                                ("load", ("load_offset", v_tmp2_l[u], v_idx_l[u], lane))
                            )
                        body.append(
                            (
                                "flow",
                        ...
                        body.append(("valu", ("^", v_val_l[u], v_val_l[u], v_tmp4_l[u])))
                    body.extend(
                        self.build_hash_vec_multi(
                            v_val_l,
                            v_tmp1_l,
            ...
            def emit_group(vec_count, base_offset_const, base_is_zero=False):
                ...
                starts = (0, 8, 1, 9, 2, 10, 3, 11, 4, 12, 5, 13, 6, 14, 7, 15)
                for round_idx in range(rounds):
                    depth = round_depths[round_idx]
                    chunk = 1
                    for start in starts:
                        emit_hash_only_range(round_idx, depth, start, chunk)
                        if depth != 0 and depth != forest_height:
                            for u in range(start, start + chunk):
                                body.append(("valu", ("&", v_tmp1[u], v_val[u], v_one)))
                                body.append(
                                    (
                                        "valu",
            ...
            print(f"fastpath scratch_ptr={self.scratch_ptr}")
