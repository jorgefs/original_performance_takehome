...
class KernelBuilder:
    ...
    def build_kernel(
        ...
        if batch_size == 256 and rounds == 16 and forest_height == 10:
            ...
            base_val_addr2 = self.alloc_scratch("base_val_addr2")
            base_idx_addr2 = self.alloc_scratch("base_idx_addr2") if store_indices else None
            self.add(
                "alu",
                ("+", base_val_addr2, self.scratch["inp_values_p"], group_size_const),
            )
            if store_indices:
                self.add(
                    "alu",
                    ("+", base_idx_addr2, self.scratch["inp_indices_p"], group_size_const),
                )
            ...
            def emit_hash_only(round_idx: int, depth: int, vec_count: int):
                if depth == 0:
                    ...
                    for u in range(vec_count):
                        body.append(("valu", ("&", v_tmp1[u], v_idx[u], v_one)))  # b0
                        body.append(("valu", ("&", v_tmp2[u], v_idx[u], v_two)))  # b1 inverted
                        body.append(
                            (
                                "flow",
            ...
            def emit_group(
                ...
                base_offset_const,
                base_is_zero=False,
                base_val_addr_override=None,
                base_idx_addr_override=None,
            ):
                if base_is_zero:
                    base_val_addr = (
                        base_val_addr_override
                        if base_val_addr_override is not None
                        else self.scratch["inp_values_p"]
                    )
                    if store_indices:
                        base_idx_addr = (
                            base_idx_addr_override
                            if base_idx_addr_override is not None
                            else self.scratch["inp_indices_p"]
                        )
                else:
                    body.append(
                        (
            ...
            emit_group(
                ...
                group_size_const,
                base_is_zero=True,
                base_val_addr_override=base_val_addr2,
                base_idx_addr_override=base_idx_addr2,
            )
