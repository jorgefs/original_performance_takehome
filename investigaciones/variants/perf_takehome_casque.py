...
class KernelBuilder:
    ...
    def vector_const(self, val, name=None):
        # Inicialización perezosa del mapa de constantes vectoriales
        if not hasattr(self, "vec_const_map"):
            self.vec_const_map = {}
    ...
    def build_hash_vec_multi(self, v_vals, v_tmps1, v_tmps2, round, i_base, emit_debug, body):
        for hi, (op1, val1, op2, op3, val3) in enumerate(HASH_STAGES):
            vec_val1 = self.vector_const(val1)
            vec_val3 = self.vector_const(val3)

            for u in range(len(v_vals)):
                # tmp1 = val op1 C1
                body.append(("valu", (op1, v_tmps1[u], v_vals[u], vec_val1)))
                # tmp2 = val op3 C3
                body.append(("valu", (op3, v_tmps2[u], v_vals[u], vec_val3)))
            
            for u in range(len(v_vals)):
                # val = tmp1 op2 tmp2
                body.append(("valu", (op2, v_vals[u], v_tmps1[u], v_tmps2[u])))
                
            # DEBUG CRITICO: Asegurar que no se optimiza ni reordena nada aquí.
            # Esta secuencia coincide con la escalar:
            # t1 = val op1 c1
            # t2 = val op3 c3
            # val = t1 op2 t2
        ...
        for i in range(0, vec_batch_unrolled, group_size):
            ...
            for round in range(rounds):
                ...
                if depth == 0:
                    ...
                    self.build_hash_vec_multi(v_val, v_tmp1, v_tmp2, round, i, emit_debug, body)
                elif depth == 1:
                    for u in range(UNROLL):
                        if emit_debug:
                    ...
                    self.build_hash_vec_multi(
                            v_val, v_tmp1, v_tmp2, round, i, emit_debug, body
                    )
                else:
                    body.extend(
