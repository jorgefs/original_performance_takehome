#!/usr/bin/env python3
"""
Testing remaining optimization strategies:
- Strategy 2: Fused Depth-0 Processing
- Strategy 3: Precomputed Level-1 Selection (arithmetic instead of vselect)
- Strategy 5: Eliminate Redundant idx Computation
"""
...
class KernelBuilderStrategy2:
    """Strategy 2: Fused Depth-0 Processing
    Emit all XORs first for depth 0, then hash stages in waves across all vectors"""
            ...
            while True:
                ...
                for i in range(min(window_size, len(pending))):
                    if pending[i][0] == "debug":
                        first_debug = i
                        break
                scan_limit = first_debug if first_debug is not None else min(window_size, len(pending))
                if scan_limit == 0:
                    break

                prefix_reads, prefix_writes = set(), set()
                chosen_i = None
                chosen_rw = None
                for i in range(scan_limit):
                    engine, slot = pending[i]
                    reads, writes = _rw_sets(engine, slot)
                    if (writes & prefix_reads) or (writes & prefix_writes) or (reads & prefix_writes):
                        prefix_reads |= reads
    ...
    def build_hash_vec_multi(self, val_addrs, tmp1_addrs, tmp2_addrs, round, i_base, emit_debug):
        slots = []
        for stage_slots in self.build_hash_vec_multi_stages(val_addrs, tmp1_addrs, tmp2_addrs, round, i_base, emit_debug):
            slots.extend(stage_slots)
        return slots

    def build_hash_pipeline_addr(self, v_idx, v_val, v_tmp1, v_tmp2, round, i_base, emit_debug, vec_count, hash_group=3):
        slots = []
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        if batch_size == 256 and rounds == 16 and forest_height == 10:
            ...
            # Strategy 2: For depth 0, emit all XORs first, then hash stages in waves
            def emit_depth0_fused(round_idx, vec_count):
                """Emit depth 0 with fused XORs and batched hash stages"""
                # First: all XORs
                for u in range(vec_count):
                    body.append(("valu", ("^", v_val[u], v_val[u], v_root_val)))
                # Then: hash stages in waves across ALL vectors
                for hi, (op1, val1, op2, op3, val3) in enumerate(HASH_STAGES):
                    if op1 == "+" and op2 == "+" and op3 == "<<":
                        mult = 1 + (1 << val3)
                        for u in range(vec_count):
                            body.append(("valu", ("multiply_add", v_val[u], v_val[u],
                                                   self.vector_const(mult), self.vector_const(val1))))
                    else:
                        for u in range(vec_count):
                            body.append(("valu", (op1, v_tmp1[u], v_val[u], self.vector_const(val1))))
                        for u in range(vec_count):
                            body.append(("valu", (op3, v_tmp2[u], v_val[u], self.vector_const(val3))))
                        for u in range(vec_count):
                            body.append(("valu", (op2, v_val[u], v_tmp1[u], v_tmp2[u])))
            ...
            def emit_group(vec_count, base_offset_const, base_is_zero=False):
                ...
                for round_idx in range(rounds):
                    ...
                    if depth == 0:
                        # Strategy 2: Fused depth-0 processing
                        emit_depth0_fused(round_idx, vec_count)
                    else:
                        for start in starts:
                            count = min(1, vec_count - start)
...
class KernelBuilderStrategy3:
    """Strategy 3: Precomputed Level-1 Selection
    Replace vselect with arithmetic: node = level1_left + bit * (level1_right - level1_left)
    Trades 1 flow op for 2 valu ops. Since flow (vselect) is bottleneck at depth 1,
    this might help if valu has spare capacity."""
    ...
    # Same build method as Strategy2...
    def build(self, slots, vliw=False):
        if not vliw:
            return [{e: [s]} for e, s in slots]
    ...
    def build_kernel(self, forest_height, n_nodes, batch_size, rounds):
        if batch_size == 256 and rounds == 16 and forest_height == 10:
            ...
            v_level1_left = self.alloc_scratch("v_level1_left", length=VLEN)
            v_level1_right = self.alloc_scratch("v_level1_right", length=VLEN)
            v_level1_delta = self.alloc_scratch("v_level1_delta", length=VLEN)  # Strategy 3: precompute delta
            v_level2 = [self.alloc_scratch(f"v_level2_{i}", length=VLEN) for i in range(4)]
            v_forest_values_p = self.alloc_scratch("v_forest_values_p", length=VLEN)
            v_base_plus1 = self.alloc_scratch("v_base_plus1", length=VLEN)
            v_base_minus1 = self.alloc_scratch("v_base_minus1", length=VLEN)
            ...
            init.append(("valu", ("vbroadcast", v_level1_left, level1_left)))
            init.append(("valu", ("vbroadcast", v_level1_right, level1_right)))
            # Strategy 3: Precompute delta = level1_right - level1_left
            init.append(("valu", ("-", v_level1_delta, v_level1_right, v_level1_left)))
            init.append(("valu", ("vbroadcast", v_level2[0], level2_vals[0])))
            init.append(("valu", ("vbroadcast", v_level2[1], level2_vals[1])))
            init.append(("valu", ("vbroadcast", v_level2[2], level2_vals[2])))
            init.append(("valu", ("vbroadcast", v_level2[3], level2_vals[3])))
            ...
            def emit_hash_only_range(round_idx, depth, start, count):
                ...
                if depth == 0:
                    ...
                    # Strategy 3: Replace vselect with arithmetic
                    # Original: vselect(tmp, cond, v_level1_right, v_level1_left)
                    # = v_level1_left + cond * (v_level1_right - v_level1_left)
                    # = v_level1_left + cond * v_level1_delta
                    # But cond is 0 or 1, so we need multiply
                    for u in range(count):
                        body.append(("valu", ("&", v_tmp1_l[u], v_val_l[u], v_one)))  # bit = val & 1
                        body.append(("valu", ("+", v_idx_l[u], v_base_plus1, v_tmp1_l[u])))
                        # node = level1_left + bit * delta
                        body.append(("valu", ("*", v_tmp2_l[u], v_tmp1_l[u], v_level1_delta)))  # bit * delta
                        body.append(("valu", ("+", v_tmp1_l[u], v_level1_left, v_tmp2_l[u])))  # left + bit*delta
                        body.append(("valu", ("^", v_val_l[u], v_val_l[u], v_tmp1_l[u])))
                    body.extend(self.build_hash_vec_multi(v_val_l, v_tmp1_l, v_tmp2_l, round_idx, start * VLEN, emit_debug))
                elif depth == 2:
                    for u in range(count):
...
if __name__ == "__main__":
    print("=" * 60)
    print("ADDITIONAL STRATEGY TESTING")
    print("=" * 60)
    print()
    print("Baseline: 1615 cycles")
    print()

    # Strategy 2: Fused Depth-0 Processing
    print("--- STRATEGY 2: Fused Depth-0 Processing ---")
    print("(Emit all XORs first, then hash stages in waves)")
    kb = KernelBuilderStrategy2()
    ...
    if error:
        ...
        diff = cycles - 1615
        sign = "+" if diff >= 0 else ""
        print(f"Result: {cycles} cycles ({sign}{diff})")
    print()

    # Strategy 3: Precomputed Level-1 Selection
    print("--- STRATEGY 3: Arithmetic instead of vselect for depth 1 ---")
    print("(Replace vselect with multiply: node = left + bit * delta)")
    kb = KernelBuilderStrategy3()
    cycles, error = test_kernel(kb)
    if error:
        print(f"ERROR: {error}")
    else:
