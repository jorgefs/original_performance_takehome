# Fix chunk=4 by allocating v_tmp3 per vector instead of shared
...
new_tmp3 = '''            CHUNK_SIZE = 4  # For chunk=4
            v_tmp3 = [
...
# 3. Change chunk from 1 to 4
content = content.replace('chunk = 1', 'chunk = 4')

# 4. Fix starts tuples - with chunk=4, we need 1/4 the starts (every 4th start)
# Original starts cover all 32 vectors individually, but with chunk=4 each start processes 4 vectors
...
# For chunk=4: use only indices divisible by 4 as starts (8 starts * 4 vectors each = 32 vectors)
new_starts_deep = '''                    if depth > 2:
                        # chunk=4: 8 starts, each processes 4 vectors
                        starts = (
...
with open('perf_takehome_chunk4.py', 'w', encoding='utf-8') as f:
    f.write(content)

print('Created: perf_takehome_chunk4.py')
print('Changes:')
print('  - v_tmp3_shared -> v_tmp3[u] per vector')
print('  - chunk = 4')
print('  - starts reduced to 8 (indices divisible by 4)')
